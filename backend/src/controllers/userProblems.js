...
const updateProblem = async (req, res) => {
  try {
    ...
    if (referenceSolution && referenceSolution.length > 0 && visibleTestCases) {
      for (const { language, completeCode } of referenceSolution) {
        //source_code
        //language_id
        //stdin
...
const getProblemById = async (req, res) => {
  try {
    ...
    getProblem.starterCode = getProblem.starterCode.map((codeObj) => ({
      ...codeObj,
      initialCode: codeObj.initialCode.replace(/\\n/g, "\n"),
    }));

    getProblem.referenceSolution = getProblem.referenceSolution.map(
      (codeObj) => ({
        ...codeObj,
        completeCode: codeObj.completeCode.replace(/\\n/g, "\n"),
      })
    );
...
const getAllProblems = async (req, res) => {
  try {
    //same here :   .select
    const getProblemss = await Problem.find({}).select(
      "_id title difficulty tags"
    );
    if (getProblemss.length === 0)
