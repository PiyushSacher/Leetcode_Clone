...
const ProblemPage = () => {
    ...
    const [isSubmitting, setIsSubmitting] = useState(false); // For run/submit buttons
    ...
    useEffect(() => {
        const fetchProblem = async () => {
            ...
            try {
                ...
                const starter = data?.startCode?.find(s => s.language.toLowerCase() === selectedLanguage.toLowerCase())?.initialCode;
    ...
    useEffect(() => {
        if (problem) {
            const starter = problem?.startCode?.find(s => s.language.toLowerCase() === selectedLanguage.toLowerCase())?.initialCode;
            setCode(starter || `// No starter code for ${selectedLanguage}`);
        }
    ...
    const onRun = async () => {
        ...
        try {
            ...
            setRunResult({ status: "error", message: error.response?.data?.message || error.message });
        }
    ...
    const onSubmit = async () => {
        ...
        setRunResult(null);
        try {
            // Calls POST /submission/submit/:id
            ...
            setSubmitResult({ status: "error", message: error.response?.data?.message || error.message });
        }
        setIsSubmitting(false);
    };
    ...
    if (!problem) {
        return <div className="min-h-screen bg-gray-900 text-red-400 flex items-center justify-center">
                    ...
                    {user && (
                        <div className="dropdown dropdown-end">
                            ...
                            <ul tabIndex={0} className="mt-3 p-2 shadow menu menu-sm dropdown-content bg-gray-800 rounded-box w-52 z-50">
                                <li><NavLink to="/">All Problems</NavLink></li>
            ...
            <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-2 p-2 overflow-auto">
                ...
                <div className="bg-gray-800 rounded-lg shadow-lg flex flex-col overflow-auto">
                    ...
                    <div className="flex border-b border-gray-700 flex-shrink-0">
                        <button 
                            className={`${tabBtn} ${activeLeftTab === 'description' ? activeTabBtn : inactiveTabBtn}`}
                            onClick={() => setActiveLeftTab('description')}
                        >
                            Description
                        </button>
                        <button 
                            className={`${tabBtn} ${activeLeftTab === 'submissions' ? activeTabBtn : inactiveTabBtn}`}
                    ...
                    <div className="p-5 overflow-y-auto">
                        ...
                        {activeLeftTab === 'submissions' && (
                            <div>
                                ...
                                <p className="text-gray-400 mt-4">You have no submissions for this problem yet.</p>
                ...
                <div className="bg-gray-800 rounded-lg shadow-lg flex flex-col overflow-auto">
                    ...
                    <div className="flex-shrink-0 border-t border-gray-700">
                        ...
                        <div className="flex border-b border-gray-700">
                            <button 
                                className={`${tabBtn} ${activeBottomTab === 'testcases' ? activeTabBtn : inactiveTabBtn}`}
                                onClick={() => setActiveBottomTab('testcases')}
                            >
                                Test Cases
                            </button>
                            <button 
                                className={`${tabBtn} ${activeBottomTab === 'result' ? activeTabBtn : inactiveTabBtn}`}
                                ...
                                <div>
                                    ...
                                    {runResult && (
                                        <div className="space-y-2">
                                            {Array.isArray(runResult.output) ? (
                                                runResult.output.map((result, index) => (
                                                    <div key={index}>
                                                        <p className={`font-medium text-sm ${result.status_id === 3 ? 'text-green-400' : 'text-red-400'}`}>
                                                        ...
                                                        {result.status_id !== 3 && (
                                                            <pre className="text-xs font-mono bg-gray-700 p-2 rounded-md mt-1">
                                                                ...
                                                                {result.compile_output && `Compile Error: ${result.compile_output}`}
                                                            </pre>
                                                        )}
                                                    </div>
                                                ))
                                            ) : (
                                                <pre className="text-sm font-mono text-red-400">{runResult.message || "An error occurred."}</pre>
                                            )}
                                        </div>
                                    )}
                                    ...
                                    {submitResult && (
                                        <div className={`p-3 rounded-md ${submitResult.status === 'accepted' ? 'bg-green-900/50' : 'bg-red-900/50'}`}>
                                            <p className={`font-bold text-lg ${submitResult.status === 'accepted' ? 'text-green-400' : 'text-red-400'}`}>
                                            ...
                                            <p className="text-xs text-gray-400 mt-2">
                                                Passed: {submitResult.testCasesPassed} / {submitResult.testCasesTotal}
                                            </p>
                                        </div>
                                    )}
                                </div>
                    ...
                    {/* Run/Submit Buttons */}
                    <form onSubmit={handleSubmit(onSubmit)} className="p-2 border-t border-gray-700 flex-shrink-0">
